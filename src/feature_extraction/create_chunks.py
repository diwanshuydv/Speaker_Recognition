...
def process_audio_file(speaker, audio_file, speaker_path,
                       speaker_output_path, chunk_duration):
    """
    ...
    for chunk_idx, start_sample in enumerate(
        ...
        chunk_filename = (
            f"{speaker}_{os.path.splitext(audio_file)[0]}"
            f"_chunk{chunk_idx}.wav"
        )
...
def create_audio_chunks(input_dir, output_dir,
                        chunk_duration=2, max_threads=None):
    """
    Splits all audio files in each speaker's folder into chunks
